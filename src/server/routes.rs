...
async fn app_icon(
    ...
    static_asset_bytes("image/png", include_bytes!("../static/icon.png"))
}

async fn app_logo(
    State(_state): State<AppState>,
) -> Result<impl IntoResponse, (StatusCode, String)> {
    static_asset_bytes("image/png", include_bytes!("../static/logo.png"))
}
...
async fn api_folder(
    ...
    let base_url = helpers::request_base_url(&headers, &state.base);
...
async fn api_folder_m3u8(
    ...
    let base = helpers::request_base_url(&headers, &state.base);
    let body = crate::playlist::render_m3u8(&base, &state.root, &tracks);
    Ok((
        [
...
async fn api_artwork(
    ...
    let track = lib
        .tracks()
        .iter()
        .find(|t| t.metadata.artwork_id.as_ref() == Some(&id))
        .ok_or((StatusCode::NOT_FOUND, String::new()))?;

    let art = lib
        .extract_artwork(&track.path)
        .ok_or((StatusCode::NOT_FOUND, String::new()))?;
